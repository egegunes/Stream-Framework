...
class BaseManagerTest(unittest.TestCase):
    ...
    def test_add_user_activity(self):
        assert (
            self.manager.get_user_feed(self.actor_id).count() == 0
        ), "the test feed is not empty"

        with patch.object(
            self.manager, "get_user_follower_ids", return_value={None: [1]}
        ) as get_user_follower_ids:
    ...
    def test_batch_import_errors(self):
        ...
        with patch.object(
            ...
            batch = partial(self.manager.batch_import, self.actor_id, [activity], 10)
    ...
    def test_add_remove_user_activity(self):
        user_id = 42
        assert (
            self.manager.get_user_feed(user_id).count() == 0
        ), "the test feed is not empty"
    ...
    def test_add_user_activity_fanout(self):
        ...
        with patch.object(
            self.manager, "get_user_follower_ids", return_value=followers
        ) as get_user_follower_ids:
            self.manager.add_user_activity(user_id, self.activity)
            get_user_follower_ids.assert_called_with(user_id=user_id)
    ...
    def test_follow_unfollow_user(self):
        ...
        with patch.object(
            self.manager, "get_user_follower_ids", return_value={}
        ) as get_user_follower_ids:
        ...
        self.manager.unfollow_user(follower_user_id, target_user_id, async_=False)
